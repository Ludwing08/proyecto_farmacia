...
public class Metodos_Ventas {
    ...
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    public int ActualizarDatos(String ID, String Descripcion, String Categoria, int Cantidad, float Costo, float Precio, String Estado) {
        String SQL = "UPDATE producto SET pro_Descripcion = '" + Descripcion + "', pro_Categoria = '" + Categoria + "', pro_Cantidad = '" + Cantidad + "', pro_Costo = '" + Costo + "', pro_Precio = '" + Precio + "', pro_Estado = '" + Estado + "' WHERE pro_ID = " + ID;
        int resultado = 0;
    ...
    //********************************************************************************************
    public int GuardarVentas(String Fecha, String VentaTotal, String IVA, String SubTotal, String Total, String Estado, String IdCliente, String IdEmpleado, String IdTipoComprobante) {
        int resultado = 0;
        Connection conexion = null;

        String sentencia_guardar = "INSERT INTO ventas (Fecha, VentaTotal, SubTotal, IVA, Total, Estado, IdCliente, IdEmpleado, IdTipoComprobante) VALUES (?,?,?,?,?,?,?,?,?)";

        try {
            conexion = ConexionBD.conectar();
            GP = conexion.prepareStatement(sentencia_guardar);                        
            GP.setString(1, Fecha);
            GP.setString(2, VentaTotal);
            GP.setString(3, SubTotal);    
            GP.setString(4, IVA);    
            GP.setString(5, Total);
            GP.setString(6, Estado);
            GP.setString(7, IdCliente);
            GP.setString(8, IdEmpleado);
            GP.setString(9, IdTipoComprobante);
            resultado = GP.executeUpdate();
            if (resultado > 0) {
            }
            conexion.close();
    ...
    public int GuardarDetalleVentas(String IdVenta, String IdProducto, String Cantidad, String Costo, String Precio, String Total) {
        ...
        String sentencia_guardar = "INSERT INTO detalleventa (IdVenta, idProducto, Cantidad, Costo, Precio, Subtotal) VALUES (?,?,?,?,?,?)";
